...
def main(output_path: str = "out.py") -> str:
    """
    前処理（曖昧表現の調整）→ query.py を CLI 実行（引数2つ） → out.py の存在確認 → ログ保存
    戻り値: explanation(str) ＝ query.py の標準出力（抜粋）
    """
    # 指示テキスト（前処理）
    ...
    if not query_script:
        raise FileNotFoundError("graphrag-gpt-main\query.py が見つかりません。run_pipeline.py と同じディレクトリに置いてください。")
    ...
    if (proc.returncode != 0) or (not new_ready):
        ...
        if explanation: err_msg.append("--- stdout (query.py) ---\n" + explanation)
        if stderr_text: err_msg.append("--- stderr (query.py) ---\n" + stderr_text)
        msg = "\n".join(err_msg)
        print(msg)
