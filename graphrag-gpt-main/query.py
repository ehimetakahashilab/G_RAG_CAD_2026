...
GRAPH_SEARCH_TOP_K = 100      # グラフ検索の最大結果数
...
def llm_response_to_text(response) -> str:
    """
    LangChain の AIMessage / Gemini 応答を安全に文字列化する。
    response.content が str の場合も list の場合も吸収する。
    """
...
def fix_bracket_code(
    ...
    llm=None
        ...
        response = llm.invoke(prompt)
        response_text = llm_response_to_text(response)

        script_match = re.search(PYTHON_CODE_BLOCK_PATTERN, response_text, re.DOTALL)
        if script_match:
...
def save_script_file(file_path: str, script_code: str, attempt: Optional[int] = None, keep_attempts: bool = True) -> str:
    """
    スクリプトをファイルに保存する
            ...
            if output_path:
                try:
                    ...
                    if script_code:
                        ...
                        file_path=save_script_file(output_path, script_code, attempt, keep_attempts)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # メイン処理
        filename=process_generation_loop(
    ...
    print("__QUERY_RESULT__=" + json.dumps({
        "filename": filename
    }, ensure_ascii=False))
